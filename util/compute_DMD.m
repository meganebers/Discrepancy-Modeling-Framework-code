function [Phi,omega,lambda,b,Xdmd,Atilde,U_r] = compute_DMD(X1,X2,r,dt)
% function [Phi,omega,lambda,b,Xdmd] = DMD(X1,X2,r,dt)
% Computes the Dynamic Mode Decomposition of X1, X2
% 
% Copied from Kutz et al. Textbook
%
% INPUTS: 
% X1 = X, data matrix
...
%% DMD
[U, S, V] = svd(X1, 'econ');
% Auto truncation rank
if r==0
% Apply optimal hard threshold from HAVOK code (LORENZ_GEN.m)
    beta = size(X1,1)/size(X1,2);
    thresh = optimal_SVHT_coef(beta,0) * median(diag(S));
